...
ytrain_matrix = np.eye(Output)[ytrain]
ytest_matrix = np.eye(Output)[ytest]
...
def nn(theta):
    ...
    # Unroll thetas
    theta1 = theta[:(n+1)*Hidden].reshape((Hidden,(n+1))) # [Hiddden X n + 1]
    theta2 = theta[(n+1)*Hidden:].reshape((Output,(Hidden+1))) # [Output X Hidden + 1]

    # Forward propagation
    a1 = np.concatenate((np.ones((1,mtrain)).T,Xtrain),axis=1) # [m X n + 1] = [200X3]
    z2 = a1.dot(theta1.T) # [m X n + 1] = [200X3]
    a2 = np.concatenate((np.ones((1,mtrain)).T,sigmoid(z2)),axis=1) # [m X Hidden + 1] = [200X4] (the bias unit is also added here, hence the + 1!)
    ...
    # Backpropagation
    d3 = (a3 - ytrain_matrix) # [m X Output] = [200X2]
    d2 = (a3 - ytrain_matrix).dot(theta2) * (a2*(1-a2)) # [m X Hidden] = [200X3]
    D1 = d2[:,1:].T.dot(a1)/mtrain # [Hidden X n + 1] = [3X3]
    D2 = d3.T.dot(a2)/mtrain # [n X Hidden + 1] = [2X4]
    if Lambda!=0:
...
opt = fmin_cg(cost, x0=theta_init, fprime=grad, full_output=1, disp=1);
theta = opt[0]
